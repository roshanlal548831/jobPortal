          ...
          allAppliedJobs.length <= 0 ? <span className='text-red-600'>You have not applied any job.</span> : allAppliedJobs.map((item,i)=> {
            return (
               <TableRow key={i}>
                   ...
                   <TableCell className="text-right"><Badge className={`${item?.status === "rejected"? "bg-red-400" : item?.status === "pending" ? "bg-gray-400" : "bg-green-500"} text-white rounded-xl`}>{item?.status}</Badge></TableCell>
               </TableRow>
            )
           })
           }
